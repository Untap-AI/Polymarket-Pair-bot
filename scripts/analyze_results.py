...
async def run_report(
    ...
    # --- Overall ---
    stats = await get_overall_stats(db_path, parameter_set_id, crypto_asset, date_after)
    ...
    # --- By Asset ---
    by_asset = await get_stats_by_asset(db_path, parameter_set_id)
    if by_asset:
    ...
    # --- TTP Distribution ---
    ttp_dist = await get_time_to_pair_distribution(db_path, parameter_set_id)
    if ttp_dist:
    ...
    # --- By First Leg ---
    by_leg = await get_stats_by_first_leg(db_path, parameter_set_id)
    if by_leg:
    ...
    # --- By Market Phase ---
    by_phase = await get_stats_by_market_phase(db_path, parameter_set_id)
    if by_phase:
    ...
    by_minute = await get_stats_by_market_minute(db_path, parameter_set_id)
    if by_minute:
    ...
    by_bucket = await get_stats_by_time_bucket(db_path, parameter_set_id)
    if by_bucket:
    ...
    # --- MAE Analysis ---
    mae = await get_mae_analysis(db_path, parameter_set_id)
    ...
    # --- Spread Analysis ---
    spread = await get_spread_analysis(db_path, parameter_set_id)
    ...
    # --- By Reference Regime ---
    by_regime = await get_stats_by_reference_regime(db_path, parameter_set_id)
    if by_regime:
    ...
    # --- Pair Cost ---
    by_cost = await get_pair_cost_distribution(db_path, parameter_set_id)
    if by_cost:
    ...
    # --- Failure Analysis ---
    failures = await get_failure_analysis(db_path, parameter_set_id)
    ...
    # --- Near Miss ---
    near = await get_near_miss_analysis(db_path, parameter_set_id)
    if near.get("total"):
    ...
    consistency = await get_cross_market_consistency(db_path, parameter_set_id)
    if consistency:
    ...
    param_cmp = await get_parameter_comparison(db_path)
    if len(param_cmp) > 1:
    ...
    proj = await get_profitability_projection(db_path, parameter_set_id)
